...
function plugin(opts) {
  ...
  return function(typesmith, done){
    ...
    var config = Object.assign({}, defaults, typesmith.config['typesmith-autoparent'], opts);
    ...
    Object.keys(typesmith.db).forEach( function(uid) {
      ...
      var type_config = Object.assign({}, config, record_type_config);
      ...
      if (parent_record) {
        ...
        var parent_type_config = Object.assign({}, config, parent_type_config);

        // start with the subgroup_by_key property.
        var subgroup_by_key = parent_type_config.subgroup_by_key;
        ...
        if (subgroup_name && subgroup_type)
        {
          ...
          if (!subgroup_record)
          {
            ...
            // TODO: we probably need to set up the new subgroup_type in the types config, so that they
            // don't get spit out on their own pages in mixtape.
          }
